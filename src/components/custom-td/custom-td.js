...
export default class CustomTD extends Component {
  ...
  setValue = (newValue, id) => {
    this.setState(
      ...
      () =>
        console.log(
          'NewValue = ' + this.state.value + ' thisId = ' + this.state.id
        )
    );
  };
  ...
  render() {
    ...
    return (
      <td
        ...
        action={action}
        onDoubleClick={() =>
          action === 'input' && this.state.input === false && counter === 0
            ? this.setState(
                {
                  input: true
                },
      ...
      >
        <>
          {this.state.input ? (
            ...
            action !== 'sum' && ( this.state.value || value)
          )}
        </>
...
class SumValues extends Component {
  ...
  sumValues = (range, id) => {
    let sumValue = 0;
    range.map(id => {
      sumValue += +this.props.getCellValue(id);
    });
